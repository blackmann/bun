void GlobalObject::initGeneratedLazyClasses() {
    ...
    m_JSStringContaining.initLater(
              [](LazyClassStructure::Initializer& init) {
                 init.setPrototype(WebCore::JSStringContaining::createPrototype(init.vm, reinterpret_cast<Zig::GlobalObject*>(init.global)));
                 init.setStructure(WebCore::JSStringContaining::createStructure(init.vm, init.global, init.prototype));
...
void GlobalObject::visitGeneratedLazyClasses(GlobalObject *thisObject, Visitor& visitor)
{
      ...
      thisObject->m_JSStringContaining.visit(visitor);  visitor.append(thisObject->m_JSStringContainingSetterValue);
